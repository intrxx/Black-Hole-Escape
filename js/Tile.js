        ...
        this.sprite.on('pointerdown', () => {
			...
			if(this.PawnBase == null && this.scene.player1.numberOfMoves > 0)
			{
                ...
                else if((this.scene.player1.bIsFirstTilePlaced = true && this.PawnBase == null) && ((OldXindex == this.indexX && OldYindex + 1 == this.indexY) ||  (OldYindex == this.indexY && OldXindex + 1 == this.indexX) || (OldXindex == this.indexX && OldYindex - 1 == this.indexY) ||  (OldYindex == this.indexY && OldXindex - 1 == this.indexX)))
                {
                    ...
                    this.scene.boardArray[this.indexX][this.indexY].bIsTaken = true;
                    console.log(this.CheckHowManyMovesPossible())
    ...
    CheckHowManyMovesPossible()
        ...
        let numberOfMovesLeft = 0;
            for(let i = 0; i < 7; i++)
            {
                for(let j = 0; j < 7; j++)
                {
                    if(this.scene.boardArray[j][i].bIsTaken == false)
                    {
                        if((j+1 <= 6 && this.scene.boardArray[j+1][i].bIsTaken == false) || 
                           (j-1 >= 0 && this.scene.boardArray[j-1][i].bIsTaken == false) || 
                           (i-1 >= 0 && this.scene.boardArray[j][i-1].bIsTaken == false) || 
                           (i+1 <= 6 && this.scene.boardArray[j][i+1].bIsTaken == false)) 
                       {
                         numberOfMovesLeft++;
                       } 
                    }   
